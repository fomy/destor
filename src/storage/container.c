...
static BOOL container_overflow_predict(Container* container, int32_t length) {
	...
	if ((CONTAINER_META_ENTRY_SIZE + length + container_size(container)) > CONTAINER_SIZE)
		return TRUE;
	return FALSE;
}
...
Fingerprint *container_get_all_fingers(Container* container) {
    Fingerprint* fingers = (Fingerprint*)malloc(sizeof(Fingerprint)*container->chunk_num);
	int32_t num = 0;
	GHashTableIter iter;
...
int32_t container_add_chunk(Container* container, Chunk* chunk) {
	...
	memcpy(container->data + container->data_size, chunk->data,
			chunk->length);
