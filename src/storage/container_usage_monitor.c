...
static double container_usage_add(ContainerUsage* cntnr_usg, Fingerprint* fngrprnt, int32_t chnk_lngth){
...
ContainerUsageMonitor* container_usage_monitor_new(){
    ContainerUsageMonitor* cntnr_usg_mntr 
        = (ContainerUsageMonitor*)malloc(sizeof(ContainerUsageMonitor));

    cntnr_usg_mntr->dense_map = 
        g_hash_table_new_full(g_int_hash, g_int_equal, NULL, container_usage_free);
    cntnr_usg_mntr->sparse_map 
        = g_hash_table_new_full(g_int_hash, g_int_equal, NULL, container_usage_free);
...
void container_usage_monitor_update(ContainerUsageMonitor* cntnr_usg_mntr, 
    ...
    ContainerUsage* cntnr_usg = g_hash_table_lookup(cntnr_usg_mntr->dense_map, &cntnr_id);
    if(cntnr_usg){
        container_usage_add(cntnr_usg, NULL, len);
    }else{
        ContainerUsage *cntnr_usg = g_hash_table_lookup(cntnr_usg_mntr->sparse_map, &cntnr_id);
        if(!cntnr_usg){
...
int container_usage_monitor_print(ContainerUsageMonitor *cntnr_usg_mntr, int32_t job_id,
    ...
    while(g_hash_table_iter_next(&iter, &key, &value)){
        ...
        if(old_sparse_containers && g_hash_table_lookup(old_sparse_containers, &cntnr_usg->cntnr_id)){
            old_sparse_num++;
        }
        fprintf(usage_file, "%d:%d:%.2f\n", cntnr_usg->cntnr_id, cntnr_usg->read_size, 
                container_usage_calc(cntnr_usg));
    }
...
GHashTable* load_historical_sparse_containers(int32_t job_id){
    ...
    GHashTable* sparse_containers = g_hash_table_new_full(g_int_hash,g_int_equal, NULL, free);

    ContainerId id;
    int32_t size;
    float tmp;
    while(fscanf(usage_file, "%d:%d:%f", &id, &size, &tmp) != EOF){
        ContainerUsage* usage = (ContainerUsage*)malloc(sizeof(ContainerUsage));
        usage->cntnr_id = id;
