...
static int read_nseed(FILE *seed_file, Seed *start, int count){
    ...
    int i = 0;
    for(; i<count; i++){
        if(fscanf(seed_file, "%d:%d\n", &start[i].container_id, &start[i].remaining_size)!=2){
            break;
        }
    }
    return i;
...
static gint distance_cmp_func(gconstpointer a, gconstpointer b, gpointer user_data){
    ...
    return ((DistanceItem*) b)->next_position - ((DistanceItem*)a)->next_position;
}

static void distance_table_add_item(GHashTable *distance_table, ContainerId container_id, int32_t distance){
...
static GHashTable* distance_table_init(Seed* window, int window_size){
    GHashTable* distance_table = g_hash_table_new_full(g_int_hash, g_int_equal, NULL, distance_item_free);
...
static ContainerPool* container_pool_new(){
    ContainerPool* container_pool = (ContainerPool*)malloc(sizeof(ContainerPool));
    container_pool->pool = g_hash_table_new_full(g_int_hash, g_int_equal, NULL, NULL); 
...
static Seed* seed_window_slide(OptimalContainerCache *opt_cache){
    ...
    if(opt_cache->seed_file_is_end == FALSE && opt_cache->seed_count == opt_cache->window_size){
        /* read more seed */
        memmove(opt_cache->seed_buffer, &opt_cache->seed_buffer[opt_cache->window_start], sizeof(Seed)*opt_cache->window_size);
        opt_cache->window_start = 0;
        int32_t new_seed_count = read_nseed(opt_cache->seed_file,
                &opt_cache->seed_buffer[opt_cache->window_size], opt_cache->window_size);
    ...
    if(opt_cache->seed_count > opt_cache->window_size){
        opt_cache->window_size++;
        Seed *seed = &opt_cache->seed_buffer[opt_cache->window_start+opt_cache->window_size-1];
        distance_table_add_item(opt_cache->distance_table, 
                seed->container_id, opt_cache->current_distance+opt_cache->window_size);
    }

    /* neccesary */
    g_sequence_sort(opt_cache->container_pool->distance_sequence, distance_cmp_func, NULL);
...
static Container* container_pool_insert(ContainerPool *container_pool, 
    ...
    if(g_hash_table_size(container_pool->pool) >= cache_size){
        /* evict */
        GSequenceIter* begin = g_sequence_get_begin_iter(container_pool->distance_sequence);
        DistanceItem* evictor_distance = g_sequence_get(begin);
        g_sequence_remove(begin);
        evictor = g_hash_table_lookup(container_pool->pool, &evictor_distance->container_id);
        g_hash_table_remove(container_pool->pool, &evictor->id);
    }
    g_hash_table_insert(container_pool->pool, &container->id, container);

    /* descending order */
    g_sequence_insert_sorted(container_pool->distance_sequence, distance, distance_cmp_func, NULL);
    return evictor;
}
...
OptimalContainerCache* optimal_container_cache_new(int cache_size, 
    ...
    OptimalContainerCache *opt_cache = (OptimalContainerCache*)malloc(sizeof(OptimalContainerCache));
    ...
    if((opt_cache->seed_count = read_nseed(opt_cache->seed_file, opt_cache->seed_buffer, 2*window_size)) == 0){
    ...
    opt_cache->window_size = window_size < opt_cache->seed_count ? window_size : opt_cache->seed_count;
    opt_cache->window_start = 0;

    /* init distance_table */
    opt_cache->distance_table = distance_table_init(opt_cache->seed_buffer, opt_cache->window_size);
...
static Container *optimal_container_cache_lookup(OptimalContainerCache *opt_cache,
        Fingerprint *finger){
...
static Container *optimal_container_cache_insert(OptimalContainerCache *opt_cache, ContainerId container_id){
    ...
    if(evictor){
        ...
        int i = 0;
        for(;i < chunknum; ++i){
            GSequence* container_list = g_hash_table_lookup(opt_cache->map, &fingers[i]);
            g_sequence_remove(g_sequence_lookup(container_list,
                        evictor, container_cmp_des, NULL));
        ...
        DistanceItem *evict_distance = g_hash_table_lookup(opt_cache->distance_table,
                &evictor->id);
        if((evict_distance->next_position == INFINITE)){
            g_hash_table_remove(opt_cache->distance_table, &evict_distance->container_id);
        }
    ...
    int i = 0;
    for(;i < num; ++i){
        GSequence* container_list = g_hash_table_lookup(opt_cache->map, &nfingers[i]);
        ...
        g_sequence_insert_sorted(container_list, required_container, container_cmp_des, NULL);
    }
    free(nfingers);
...
Chunk* optimal_container_cache_get_chunk(OptimalContainerCache* opt_cache,
    ...
    /* slide window */
    if(opt_cache->active_seed == NULL || opt_cache->active_seed->remaining_size == 0){
        /* next container */
