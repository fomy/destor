...
int update_job_volume_des(JobVolume* volume) {
    ...
    fwrite(&volume->job.job_id, sizeof(volume->job.job_id), 1, volume->job_meta_file);
    fwrite(&volume->job.is_del, sizeof(volume->job.is_del), 1, volume->job_meta_file);
    fwrite(&volume->job.backup_path, sizeof(volume->job.backup_path), 1, volume->job_meta_file);
    fwrite(&volume->job.file_num, sizeof(volume->job.file_num), 1, volume->job_meta_file);
    fwrite(&volume->job.chunk_num, sizeof(volume->job.chunk_num), 1, volume->job_meta_file);
    return SUCCESS;
}

int load_job_volume_des(JobVolume* volume) {
    fseek(volume->job_meta_file, 0, SEEK_SET);
    fread(&volume->job.job_id, sizeof(volume->job.job_id), 1, volume->job_meta_file);
    fread(&volume->job.is_del, sizeof(volume->job.is_del), 1, volume->job_meta_file);
    fread(&volume->job.backup_path, sizeof(volume->job.backup_path), 1, volume->job_meta_file);
    fread(&volume->job.file_num, sizeof(volume->job.file_num), 1, volume->job_meta_file);
    fread(&volume->job.chunk_num, sizeof(volume->job.chunk_num), 1, volume->job_meta_file);
    return SUCCESS;
}
...
int jvol_append_meta(JobVolume *volume, Recipe* recipe){
    int count = fwrite(&recipe->fileindex, sizeof(recipe->fileindex), 1, volume->job_meta_file);
    count += fwrite(&recipe->filename, sizeof(recipe->filename), 1, volume->job_meta_file);
    count += fwrite(&recipe->chunknum, sizeof(recipe->chunknum), 1, volume->job_meta_file);
    count += fwrite(&recipe->filesize, sizeof(recipe->filesize), 1, volume->job_meta_file);
    if(count != 4){
...
int jvol_append_fingerchunk(JobVolume *volume, FingerChunk* fchunk){
    int count = fwrite(&fchunk->fingerprint, sizeof(Fingerprint), 1, volume->job_data_file);
    count += fwrite(&fchunk->container_id, sizeof(fchunk->container_id), 1, volume->job_data_file);
    count += fwrite(&fchunk->length, sizeof(fchunk->length), 1, volume->job_data_file);
    char valid='\t';
...
FingerChunk* jvol_read_fingerchunk(JobVolume *volume){
    ...
    int count = fread(&fchunk->fingerprint, sizeof(Fingerprint), 1, volume->job_data_file);
    count += fread(&fchunk->container_id, sizeof(fchunk->container_id), 1, volume->job_data_file);
    count += fread(&fchunk->length, sizeof(fchunk->length), 1, volume->job_data_file);
    char valid;
...
Recipe* read_next_recipe(JobVolume* volume) {
    ...
    int count = fread(&recipe->fileindex, sizeof(recipe->fileindex), 1, volume->job_meta_file);
    count += fread(&recipe->filename, sizeof(recipe->filename), 1, volume->job_meta_file);
    int32_t chunknum=0;
    count += fread(&chunknum, sizeof(recipe->chunknum), 1, volume->job_meta_file);
    count += fread(&recipe->filesize, sizeof(recipe->filesize), 1, volume->job_meta_file);
